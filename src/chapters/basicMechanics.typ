...
Below is power decreasing pseudocode @powerfig taken directly from decompiled UCN : Recode.
...
We can write down the expression more clearly (see Eq. @eqpower1).

$ p_(n e w) = p_(o l d) - 4.1u(1 - h m * 0.05) $ <eqpower1>
...
With all of that we got (see Eq. @eqpower2, @eqpower3).

$ p_(h m)(u) = -4.1u * (0.95 * 60) / 1000 = -0.2337u $ <eqpower2>
$ p_(n m)(u) =- 4.1u * 60 / 1000 = -0.246u $ <eqpower3>
...
Below is power generator pseudocode @powergenfig taken directly from decompiled UCN : Recode
...
We can write down the expression more clearly (see Eq. @eqpowergen).

$ p_(n e w) = p_(o l d) + 4.5 / 5 $ <eqpowergen>

And the same way converting units, we will get amount or energy power gen will generate for 1 second (see Eq. @eqpowergen2).

$ p g = 4.5 / 5 * 60 / 1000 = 0.054 $ <eqpowergen2>
...
So here his progress gain pseudocode @mmgainfig taken directly from decompiled UCN : Recode
...
In expression form (Eq. @eqmmgain).

$ m_1 = m_0 + 0.015n * (A I)/20 $ <eqmmgain>

Using that we can define function $m^(+)(n, a)$ - Music Man progression gain on $a$ ai value, from 1 second of $n, (n>0)$ noice value(Eq. @eqmmgain2):

$ m^(+)(n, a) = 0.015 * 60 * n * a/20 = 0.9n * a/20 $ <eqmmgain2>

There is no difference bettween normal and hard modes for Music Man progression. 

Now about decreasing his progress. Here his progress decrease pseudocodes, generic @mmdecreasenmfig and hard mode additional @mmdecreasehmfig taken directly from decompiled UCN : Recode.
...
We are gonna to define constants $m^(-)_(n m)$ and $m^(-)_(h m)$, the amount of Music Man progress decrease (with 0 noice) for 1 seconds on normal and hard modes respectively(Eq. @eqmmdecrease).

$ m^(-)_(n m) = -60 * 0.03 = -1.8 \ m^(-)_(h m) = m^(-)_(n m) + 60 * 0.025 = -1.8 + 1.5 = -0.3 $ <eqmmdecrease>

And we can make generic functions $m(n, a)$ - Music Man with $a$ AI progression change for 1 second with $n$ noice for normal and hard modes(Eq. @mmprogresseq).
...
We are gonna to define constants $m^(s v)_(n m)$ and $m^(s v)_(h m)$, the amount of Music Man progress decrease with silent ventilation for 1 seconds on normal and hard modes respectively(Eq. @eqmmsv).

$ m^(s v)_(n m) = -60 * 0.015 = -0.9 \ m^(s v)_(h m) = m^(s v)_(n m) - 60 * 0.1 = -0.9 - 6 = -6.9 $ <eqmmsv>
...
Below is idle temperature increase pseudocode @tempfig taken directly from decompiled UCN : Recode.
...
So after converting units and defining only 'delta' part, we obtain the temperature change in 1 second. (Eq. @eqtempidle).

$ t_(i d l e) = 0.0167 * 60 approx 1 $ <eqtempidle>
...
Below is fan @fantempfig and AC @actempfig temperature decrease pseudocode taken directly from decompiled UCN : Recode.
...
In converted expressions(Eq @eqtempdec):

$ t_(f a n) = - 0.03 * 60 = -1.8 \ t_(A C) = - 0.04 * 60 = -2.4 $ <eqtempdec>
...
Below is heater @heatertempfig temperature increase pseudocode taken directly from decompiled UCN : Recode.
...
In converted expressions(Eq @eqheatertemp):

$ t_(h e a t) = 0.02 * 60 = 1.2 $ <eqheatertemp>
...
Below is progression subtract pseudocode for Jack-O-Chica @jackochicafig taken directly from decompiled UCN : Recode.
...
Where `Alterable Value A` - Jack-O-Chica AI value, and `Alterable Value I` is a some temperature coefficient @tempcoeffig, which is equal to $(t - 59)/60 + 1$.
